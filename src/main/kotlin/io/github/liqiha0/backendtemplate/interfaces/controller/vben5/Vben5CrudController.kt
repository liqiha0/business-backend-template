...
@Vben5Response
open class Vben5CrudController<T : AuditableAggregateRoot<T>, ID : Any>(
    private val crudRepository: JpaRepository<T, ID>,
) {

    @GetMapping("/{id}")
    @Operation(summary = "查询单个实体")
    open fun getById(@PathVariable("id") id: ID): Vben5Result<T> {
        val entity = this.crudRepository.findByIdOrNull(id) ?: return vben5Error("资源不存在", 404)
        return vben5Success(entity)
    }

    @PostMapping("/batch")
    @Operation(summary = "查询多个实体")
    fun batch(@RequestBody body: List<ID>): Vben5Result<Iterable<T>> {
        return vben5Success(this.crudRepository.findAllById(body))
    }
    ...
    open fun create(@RequestBody entity: T): Vben5Result<T> {
        return vben5Success(this.crudRepository.save(entity))
    }
    ...
    open fun update(@PathVariable id: ID, @RequestBody entity: T): Vben5Result<T> {
        val existing = crudRepository.findByIdOrNull(id) ?: return vben5Error("资源不存在")
        ...
        return vben5Success(this.crudRepository.save(existing))
    }
